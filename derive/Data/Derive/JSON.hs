...
--
-- Derive 'Text.JSON' instances. Example:
--
-- @
-- $(derives [makeJSON] [''Type])
-- @
--
...
mkRead (_, d) = let
  ...
  in
    ...
    [H.Alt M.noSrcLoc H.PWildCard (H.UnGuardedAlt readError) (H.BDecls [])]
...
mkReadRecord cn fs = H.Do $
    [H.Generator M.noSrcLoc (H.PApp (qname "JSObject") [pVar "z"])
...
mkReadRecordField cn xi (fn, _) = H.Generator M.noSrcLoc xi $
    apps (var "maybe") [
...
mkReadPlain cn fs = H.Do $
    [H.Generator M.noSrcLoc (H.PApp (qname "JSArray") [H.PList (pVars "x" fs)])
        (var "return" `H.App` var "y")] ++
    mkReadTrailer cn fs

mkReadTrailer :: String -> FieldDecl -> [Stmt]
mkReadTrailer cn fs =
    [ H.Generator M.noSrcLoc yi (var "readJSON" `H.App` xi)
